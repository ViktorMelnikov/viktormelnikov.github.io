...
(function attachConsoleProbeTests(global) {
    ...
    const DEFERRED_ERROR_CONSOLE_CALLS = [
        {method: "debug", args: ["error"], errorLabel: "error"},
        {method: "info", args: ["error"], errorLabel: "error"},
        {method: "log", args: ["error"], errorLabel: "error"},
        {method: "error", args: ["error"], errorLabel: "error"},
        {method: "trace", args: ["error"], errorLabel: "error"},
        {method: "warn", args: ["error"], errorLabel: "error"},
    ];
    ...
    function runConsoleMethodViaSetTimeout(method, args, state, testId, delay) {
        ...
        try {
            ...
            // the method reference is detached from `console`, so `this` is
            // not the console object, and it is invoked asynchronously by the
            // host. The thin wrapper only times the deferred execution; `fn`
            // is still called bare (no console binding), matching setTimeout.
            setTimeout(function deferredConsoleCall() {
                call.startedAt = now();
                call.scheduleLatencyMs = call.startedAt - call.scheduledAt;
